...
class ESPHomeMQTTPlugin(octoprint.plugin.SettingsPlugin,
        ...
        def __init__(self):
                self._logger = logging.getLogger("octoprint.plugins.esphome_mqtt")
                self._esphome_mqtt_logger = logging.getLogger("octoprint.plugins.esphome_mqtt.debug")
                self.abortTimeout = 0
        ...
        def on_settings_save(self, data):
                ...
                if self.powerOffWhenIdle:
                        self._esphome_mqtt_logger.debug("Settings saved, Automatic Power Off Enabled, starting idle timer...")
                ...
                if old_debug_logging != new_debug_logging:
                        if new_debug_logging:
                                self._esphome_mqtt_logger.setLevel(logging.DEBUG)
                        else:
                                self._esphome_mqtt_logger.setLevel(logging.INFO)
        ...
        def on_startup(self, host, port):
                ...
                esphome_mqtt_logging_hnadler = CleaningTimedRotatingFileHandler(self._settings.get_plugin_logfile_path(postfix="debug"), when="D", backupCount=3)
                esphome_mqtt_logging_hnadler.setFormatter(logging.Formatter("[%(asctime)s] %(levelname)s: %(message)s"))
                esphome_mqtt_logging_hnadler.setLevel(logging.DEBUG)

                self._esphome_mqtt_logger.addHandler(esphome_mqtt_logging_hnadler)
                self._esphome_mqtt_logger.setLevel(logging.DEBUG if self._settings.get_boolean(["debug_logging"]) else logging.INFO)
                self._esphome_mqtt_logger.propagate = False
        ...
        def on_after_startup(self):
                ...
                if helpers:
                        if "mqtt_subscribe" in helpers:
                                ...
                                for relay in self._settings.get(["arrRelays"]):
                                        self._esphome_mqtt_logger.debug(self.generate_mqtt_full_topic(relay, "state"))
                                        self.mqtt_subscribe(self.generate_mqtt_full_topic(relay, "state"), self._on_mqtt_subscription, kwargs=dict(top=relay["topic"],relayN=relay["relayN"]))
                        if "mqtt_publish" in helpers:
                                self.mqtt_publish = helpers["mqtt_publish"]
                                self.mqtt_publish("octoprint/plugin/esphome", "OctoPrint-ESPHomeMQTT publishing.")
                                if any(map(lambda r: r["event_on_startup"] == True, self._settings.get(["arrRelays"]))):
                                        for relay in self._settings.get(["arrRelays"]):
                                                self._esphome_mqtt_logger.debug("powering on {} due to startup.".format(relay["topic"]))
                                                self.turn_on(relay)
                ...
                self.abortTimeout = self._settings.get_int(["abortTimeout"])
                self._esphome_mqtt_logger.debug("abortTimeout: %s" % self.abortTimeout)

                self.powerOffWhenIdle = self._settings.get_boolean(["powerOffWhenIdle"])
                self._esphome_mqtt_logger.debug("powerOffWhenIdle: %s" % self.powerOffWhenIdle)

                self.idleTimeout = self._settings.get_int(["idleTimeout"])
                self._esphome_mqtt_logger.debug("idleTimeout: %s" % self.idleTimeout)
                self.idleIgnoreCommands = self._settings.get(["idleIgnoreCommands"])
                self._idleIgnoreCommandsArray = self.idleIgnoreCommands.split(',')
                self._esphome_mqtt_logger.debug("idleIgnoreCommands: %s" % self.idleIgnoreCommands)
                self.idleTimeoutWaitTemp = self._settings.get_int(["idleTimeoutWaitTemp"])
                self._esphome_mqtt_logger.debug("idleTimeoutWaitTemp: %s" % self.idleTimeoutWaitTemp)

                if self.powerOffWhenIdle:
                        self._esphome_mqtt_logger.debug("Starting idle timer due to startup")
                        self._reset_idle_timer()

        def _on_mqtt_subscription(self, topic, message, retained=None, qos=None, *args, **kwargs):
                self._esphome_mqtt_logger.debug("Received message for {topic}: {message}".format(**locals()))
                self.mqtt_publish("octoprint/plugin/esphome", "echo: " + message.decode("utf-8"))
                newrelays = []
                ...
                for relay in self._settings.get(["arrRelays"]):
                        if relay["topic"] == "{top}".format(**kwargs) and relay["relayN"] == "{relayN}".format(**kwargs) and relay["currentstate"] != message.decode("utf-8"):
                                ...
                                if relay["automaticShutdownEnabled"] == True and self._settings.get_boolean(["powerOffWhenIdle"]) and relay["currentstate"] == "ON":
                                        self._esphome_mqtt_logger.debug("Forcing reset of idle timer because {} was just turned on.".format(relay["topic"]))
        ...
        def on_event(self, event, payload):
                ...
                if event == Events.PRINT_STARTED and self.powerOffWhenIdle == True:
                        if self._abort_timer is not None:
                                ...
                                self._abort_timer = None
                                self._esphome_mqtt_logger.debug("Power off aborted because starting new print.")
                ...
                if event == Events.ERROR:
                        self._esphome_mqtt_logger.debug("Powering off enabled plugs because there was an error.")
                        for relay in self._settings.get(['arrRelays']):
                ...
                if self.powerOffWhenIdle == True and event == Events.MOVIE_RENDERING:
                        self._esphome_mqtt_logger.debug("Timelapse generation started: %s" % payload.get("movie_basename", ""))
                        self._timelapse_active = True

                if self._timelapse_active and event == Events.MOVIE_DONE or event == Events.MOVIE_FAILED:
                        self._esphome_mqtt_logger.debug("Timelapse generation finished: %s. Return Code: %s" % (payload.get("movie_basename", ""), payload.get("returncode", "completed")))
                        self._timelapse_active = False

                # Printer Connected Event
                if event == Events.CONNECTED:
                        if self._autostart_file:
                                self._esphome_mqtt_logger.debug("printer connected starting print of %s" % self._autostart_file)
                ...
                if event == Events.UPLOAD and any(map(lambda r: r["event_on_upload"] == True, self._settings.get(["arrRelays"]))):
                        if payload.get("print", False):  # implemented in OctoPrint version 1.4.1
                                self._esphome_mqtt_logger.debug(
                                        "File uploaded: %s. Turning enabled relays on." % payload.get("name", ""))
                                self._esphome_mqtt_logger.debug(payload)
                                for relay in self._settings.get(['arrRelays']):
                                        self._esphome_mqtt_logger.debug(relay)
                                        if relay["event_on_upload"] is True and not self._printer.is_ready():
                                                self._esphome_mqtt_logger.debug("powering on %s due to %s event." % (relay["topic"], event))
        ...
        def get_assets(self):
                return dict(
                        js=["js/jquery-ui.min.js","js/knockout-sortable.1.2.0.js","js/fontawesome-iconpicker.js","js/ko.iconpicker.js","js/esphome_mqtt.js"],
                        css=["css/font-awesome.min.css","css/font-awesome-v4-shims.min.css","css/fontawesome-iconpicker.css","css/esphome_mqtt.css"]
                )
        ...
        def get_template_configs(self):
                return [
                        ...
                        dict(type="sidebar", icon="plug", custom_bindings=True, data_bind="visible: filteredSmartplugs().length > 0", template="esphome_mqtt_sidebar.jinja2", template_header="esphome_mqtt_sidebar_header.jinja2")
                ]
        ...
        def on_api_command(self, command, data):
                if not Permissions.PLUGIN_ESPHOME_MQTT_CONTROL.can():
                        ...
                        for relay in self._settings.get(["arrRelays"]):
                                if relay["topic"] == "{topic}".format(**data) and relay["relayN"] == "{relayN}".format(**data):
                                        if command == "turnOff" or (command == "toggleRelay" and relay["currentstate"] == "ON"):
                                                self._esphome_mqtt_logger.debug("turning off {topic} relay {relayN}".format(**data))
                                                self.turn_off(relay)
                                        if command == "turnOn" or (command == "toggleRelay" and relay["currentstate"] == "OFF"):
                                                self._esphome_mqtt_logger.debug("turning on {topic} relay {relayN}".format(**data))
                                                self.turn_on(relay)
                if command == 'checkStatus':
                        for relay in self._settings.get(["arrRelays"]):
                                self._esphome_mqtt_logger.debug("checking status of %s relay %s" % (relay["topic"],relay["relayN"]))
                                try:
                                        self.mqtt_publish(self.generate_mqtt_full_topic(relay, "command"),"")
                                except:
                                        self._plugin_manager.send_plugin_message(self._identifier, dict(noMQTT=True))

                if command == 'checkRelay':
                        self._esphome_mqtt_logger.debug("subscribing to {topic} relay {relayN}".format(**data))
                        for relay in self._settings.get(["arrRelays"]):
                                if relay["topic"] == "{topic}".format(**data) and relay["relayN"] == "{relayN}".format(**data):
                                        self.mqtt_subscribe(self.generate_mqtt_full_topic(relay, "state"), self._on_mqtt_subscription, kwargs=dict(top="{topic}".format(**data),relayN="{relayN}".format(**data)))
                                        self._esphome_mqtt_logger.debug("checking {topic} relay {relayN}".format(**data))
                                        self.mqtt_publish(self.generate_mqtt_full_topic(relay, "command"), "")

                if command == 'removeRelay':
                        for relay in self._settings.get(["arrRelays"]):
                                if relay["topic"] == "{topic}".format(**data) and relay["relayN"] == "{relayN}".format(**data):
                                        self.mqtt_unsubscribe(self._on_mqtt_subscription,topic=self.generate_mqtt_full_topic(relay, "state"))

                if command == 'enableAutomaticShutdown':
                        self.powerOffWhenIdle = True
                        self._esphome_mqtt_logger.debug("Automatic Power Off enabled, starting idle timer.")
                ...
                if command == 'disableAutomaticShutdown':
                        ...
                        if self._abort_timer is not None:
                                ...
                                self._abort_timer = None
                        self._timeout_value = None
                        self._esphome_mqtt_logger.debug("Automatic Power Off disabled, stopping idle and abort timers.")
                ...
                if command == 'abortAutomaticShutdown':
                        if self._abort_timer is not None:
                                ...
                                self._abort_timer = None
                        self._timeout_value = None
                        self._esphome_mqtt_logger.debug("Power off aborted.")
                        self._esphome_mqtt_logger.debug("Restarting idle timer.")
                        self._reset_idle_timer()

                if command == "enableAutomaticShutdown" or command == "disableAutomaticShutdown":
                        self._esphome_mqtt_logger.debug("Automatic power off setting changed: %s" % self.powerOffWhenIdle)
        ...
        def turn_on(self, relay):
                self.mqtt_publish(self.generate_mqtt_full_topic(relay, "command"), "ON")
                if relay["sysCmdOn"]:
                ...
                if self.powerOffWhenIdle == True and relay["automaticShutdownEnabled"] == True:
                        self._esphome_mqtt_logger.debug("Resetting idle timer since relay %s | %s was just turned on." % (relay["topic"], relay["relayN"]))
                        self._waitForHeaters = False
                        self._reset_idle_timer()
        ...
        def turn_off(self, relay):
                ...
                self.mqtt_publish(self.generate_mqtt_full_topic(relay, "command"), "OFF")

        ##~~ Gcode processing hook

        def gcode_turn_off(self, relay):
                if relay["warnPrinting"] and self._printer.is_printing():
                        self._esphome_mqtt_logger.debug("Not powering off %s | %s because printer is printing." % (relay["topic"],relay["relayN"]))
                else:
                        self.turn_off(relay)
                                ...
                                for relay in self._settings.get(["arrRelays"]):
                                        if relay["topic"].upper() == topic.upper() and relay["relayN"] == relayN and relay["gcode"]:
                                                if cmd.startswith("M80"):
                                                        ...
                                                        ## t = threading.Timer(int(relay["gcodeOffDelay"]),self.mqtt_publish,[relay["topic"] + "/command/Power" + relay["relayN"], "OFF"])
        ...
        def _idle_poweroff(self):
                ...
                if (uptime()/60) <= (self._settings.get_int(["idleTimeout"])):
                        self._esphome_mqtt_logger.debug("Just booted so wait for time sync.")
                        self._esphome_mqtt_logger.debug("uptime: {}, comparison: {}".format((uptime()/60), (self._settings.get_int(["idleTimeout"]))))
                        self._reset_idle_timer()
                        return

                self._esphome_mqtt_logger.debug("Idle timeout reached after %s minute(s). Turning heaters off prior to powering off plugs." % self.idleTimeout)
                if self._wait_for_heaters():
                        self._esphome_mqtt_logger.debug("Heaters below temperature.")
                        if self._wait_for_timelapse():
                                self._timer_start()
                else:
                        self._esphome_mqtt_logger.debug("Aborted power off due to activity.")

        ##~~ Timelapse Monitoring

        def _wait_for_timelapse(self):
                self._waitForTimelapse = True
                self._esphome_mqtt_logger.debug("Checking timelapse status before shutting off power...")
                ...
                while True:
                        ...
                        self._esphome_mqtt_logger.debug("Waiting for timelapse before shutting off power...")
                        time.sleep(5)
        ...
        def _wait_for_heaters(self):
                ...
                for heater, entry in heaters.items():
                        ...
                        if temp != 0:
                                self._esphome_mqtt_logger.debug("Turning off heater: %s" % heater)
                                self._skipIdleTimer = True
                                self._printer.set_temperature(heater, 0)
                                self._skipIdleTimer = False
                        else:
                                self._esphome_mqtt_logger.debug("Heater %s already off." % heater)
                ...
                while True:
                        ...
                        for heater, entry in heaters.items():
                                ...
                                self._esphome_mqtt_logger.debug("Heater %s = %sC" % (heater,temp))
                        ...
                        self._esphome_mqtt_logger.debug("Waiting for heaters(%s) before shutting power off..." % ', '.join(heaters_above_waittemp))
                        time.sleep(5)
        ...
        def _timer_start(self):
                ...
                self._esphome_mqtt_logger.debug("Starting abort power off timer.")
        ...
        def _shutdown_system(self):
                self._esphome_mqtt_logger.debug("Automatically powering off enabled plugs.")
                for relay in self._settings.get(['arrRelays']):
        ...
        def generate_mqtt_full_topic(self, relay, prefix):
                ...
                full_topic = re.sub(r'%name%', relay["relayN"], full_topic)
        ...
        def get_update_information(self):
                return dict(
                        esphome_mqtt=dict(
                                displayName="ESPHome-MQTT",
                                displayVersion=self._plugin_version,

                                # version check: github repository
                                type="github_release",
                                user="jneilliii",
                                repo="OctoPrint-ESPHomeMQTT",
                                ...
                                # update method: pip
                                pip="https://github.com/jneilliii/OctoPrint-ESPHomeMQTT/archive/{target_version}.zip"
                        )
                )

__plugin_name__ = "ESPHome-MQTT"
__plugin_pythoncompat__ = ">=2.7,<4"

def __plugin_load__():
        global __plugin_implementation__
        __plugin_implementation__ = ESPHomeMQTTPlugin()
